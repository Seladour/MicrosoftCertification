...
static void MakeChange(int cost, int[] cashTill, int twenties, int tens = 0, int fives = 0, int ones = 0)
{
    ...
    if (changeNeeded > 0)
    {
        cashTill[0] += (attemptedChange[0] - ones);
        cashTill[1] += (attemptedChange[1] - fives);
        cashTill[2] += (attemptedChange[2] - tens);
        cashTill[3] += (attemptedChange[3] -twenties);
